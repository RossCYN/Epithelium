...
class myAugmentation(object):
    ...
    def __init__(self, train_path="/home/nic/Mycode/Unet/tissue/data/train/image_gray", 
                 label_path="/home/nic/Mycode/Unet/tissue/data/train/label_gray", 
                 merge_path="/home/nic/Mycode/Unet/tissue/merge",
                 aug_merge_path="/home/nic/Mycode/Unet/tissue/aug_merge",
                 aug_train_path="/home/nic/Mycode/Unet/tissue/aug_train",
                 aug_label_path="/home/nic/Mycode/Unet/tissue/aug_label", 
                 img_type="tif"):
    ...
    def Augmentation(self):
        ...
        for i in range(len(trains)):
            ...
            #im = Image.open(path_train + "/" + str(3) + "." + imgtype)
    ...
    def splitMerge(self):
        ...
        """
        aug_path_merge = self.aug_merge_path       # 合并增强之后的图像
        ...
        for i in range(1):
            ...
            #print(path)
            merge_imgs = glob.glob(aug_path_merge + "/*." + self.img_type)  # 所有训练图像
            savedir = path_train# + "/" + str(i)                   # 保存训练集的路径
            if not os.path.lexists(savedir):
                os.mkdir(savedir)
            savedir1 = path_label# + "/" + str(i)                   # 保存label的路径
            if not os.path.lexists(savedir1):
                os.mkdir(savedir1)
            for imgname in merge_imgs:         # rindex("/") 是返回'/'在字符串中最后一次出现的索引
...
class dataProcess(object):
    def __init__(self, out_rows, out_cols, aug_merge_path="/home/nic/Mycode/Unet/tissue/aug_merge",
                 aug_train_path="/home/nic/Mycode/Unet/tissue/aug_train",
                 aug_label_path="/home/nic/Mycode/Unet/tissue/aug_label",
                 test_path = '/home/nic/Mycode/Unet/tissue/data/test/image_gray', 
                 npy_path="/home/nic/Mycode/Unet/tissue/npydata",
                 img_type="tif"):
...
#############################################################################################################################
    def create_train_data(self):
        ...
        for indir in os.listdir(self.aug_merge_path):
            ...
            '''
        trainPath=self.aug_train_path
        labelPath=self.aug_label_path
        imgs = glob.glob(trainPath + '/*' + '.tif')
        labels = glob.glob(labelPath + '/*' + '.tif')
        imgdatas = np.ndarray((len(imgs), self.out_rows, self.out_cols, 1), dtype=np.uint8)
        imglabels = np.ndarray((len(imgs), self.out_rows, self.out_cols, 1), dtype=np.uint8)
        #TempI[:]=imgs
        TempI=glob.glob(trainPath + '/*' + '.tif')
        TempL=glob.glob(labelPath + '/*' + '.tif')
        for num in range(1):
            #imgs = glob.glob(trainPath + '/*' + '.tif')
            '''          
            I=glob.glob(train_path + '/*' + '.tif')
            J=glob.glob(label_path+'/*'+'.tif')
            for i,j in zip(I,J):
                print(i)
                print(j)
            
            #TempI=I   ERROR TempI and I will be the same one,TempI[:] will be another copy of I
            TempI[:]=I
            for imgname in I:
                TempI[int(imgname[imgname.rindex("/") +1:imgname.rindex(".tif")])] =imgname 
                
            TempJ[:]=J
            #TempJ[:]=J[:]
            for imgname in J:
                TempJ[int(imgname[imgname.rindex("/")+1:imgname.rindex(".tif")])] =imgname
        
            for i,j in zip(TempI,TempJ):
                print(i)
                print(j)
            '''
           
            i=0
            for imgname,imglabel in zip(TempI,TempL):
                ...
                Temp_imglabel=imglabel[0:int(imglabel.rindex('/'))+1]+imgname[imgname.rindex('/')+1:] 
                #label = load_img(labelPath + '/' + labelimgname, grayscale=True)
                label = load_img(Temp_imglabel, grayscale=True)
...
#################################################################################################################        
    ...
    def create_test_data(self):
        ...
        for imgname in imgs:
            TempI[int(imgname[imgname.rindex("/") +1:imgname.rindex(".tif")])] =imgname 
        i = 0
        for imgname in TempI:
            #midname = imgname[imgname.rindex("/") + 1:]   # 图像的名字
            img = load_img(imgname, grayscale=True)   # 转换为灰度图
            img = img_to_array(img)
            imgdatas[i] = img
            #if i % 100 == 0:
                #print('Done: {0}/{1} images'.format(i, len(imgs)))
